...
app.post('/api/web/feedback', async (req, res) => {
  ...
  try {
    if (dbAvailable) {
      await pool.query(
        `INSERT INTO feedback (citizen_name, citizen_nic, service, rating, comment) VALUES ($1,$2,$3,$4,$5)`,
        [citizenName, citizenNic || null, service, rating, comment || null]
      );
    } else {
  ...
  try {
    if (dbAvailable) {
      ...
      if (entry.status === 'serving') {
        return res.json({ found: true, token: entry.token, officeId: entry.office_id, service: entry.service, status: 'serving', position: 0 });
      }
      ...
      res.json({ found: true, token: entry.token, officeId: entry.office_id, service: entry.service, status: 'waiting', position: ahead });
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
